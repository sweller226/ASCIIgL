...
namespace blockmodels {
    ...
    public:
        ...
        void RegisterModel(uint32_t stateId, std::shared_ptr<const blockstate::BlockModel> model, blockstate::BlockStateRegistry& bsr);
        // Explicitly register one shared model for every state in a type.
        void RegisterModel(uint16_t typeId, std::shared_ptr<const blockstate::BlockModel> model, blockstate::BlockStateRegistry& bsr);
