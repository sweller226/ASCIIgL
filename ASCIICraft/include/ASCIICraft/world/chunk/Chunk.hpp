...
public:
    ...
    bool HasOpaqueNoCullMesh() const { return hasOpaqueNoCullMesh; }
    bool HasTransparentMesh() const { return hasTransparentMesh; }
    bool HasMesh() const { return HasOpaqueMesh() || HasOpaqueNoCullMesh() || HasTransparentMesh(); }

    ASCIIgL::Mesh* GetOpaqueMesh() const { return opaqueMesh.get(); }
    ASCIIgL::Mesh* GetOpaqueNoCullMesh() const { return opaqueNoCullMesh.get(); }
    ...
    void InvalidateMesh() {
        hasOpaqueMesh = false;
        opaqueMesh.reset();
        hasOpaqueNoCullMesh = false;
        opaqueNoCullMesh.reset();
...
private:
    ...
    bool hasOpaqueMesh;
    bool hasOpaqueNoCullMesh;
    bool hasTransparentMesh;

    std::unique_ptr<ASCIIgL::Mesh> opaqueMesh;
    std::unique_ptr<ASCIIgL::Mesh> opaqueNoCullMesh;
