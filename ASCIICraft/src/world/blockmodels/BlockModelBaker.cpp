#include <ASCIICraft/world/blockmodels/BlockModelBaker.hpp>
...
jsonutil::LoadResult<blockstate::BlockModel> BakeResolvedModel(
    ...
    if (!resolveTextureLayer) {
        return jsonutil::Fail<blockstate::BlockModel>("BakeResolvedModel: resolveTextureLayer callback is null");
    }
    ...
    for (const auto& elem : resolved.elements) {
        for (const auto& faceKv : elem.faces) {
            ...
            auto layerIdxRes = resolveTextureLayer(face.texture);
            if (!layerIdxRes.Ok()) {
                return jsonutil::Fail<blockstate::BlockModel>(
                    "BakeResolvedModel: failed to resolve texture '" + face.texture + "': " +
                    layerIdxRes.error.value_or("unknown error")
                );
            }

            const float texLayer = static_cast<float>(layerIdxRes.value.value());
