...
jsonutil::LoadResult<BlockModelDefinition> JsonModelLoader::ParseBlockModelJsonText(const std::string& jsonText, const std::string& debugName) const {
    ...
    if (root.contains("ambientocclusion")) {
        if (!root["ambientocclusion"].is_boolean()) return jsonutil::Fail<BlockModelDefinition>(debugName + ": 'ambientocclusion' must be a boolean");
        out.ambientocclusion = root["ambientocclusion"].get<bool>();
    }
            ...
            if (je.contains("faces")) {
                ...
                for (auto fit = jf.begin(); fit != jf.end(); ++fit) {
                    ...
                    if (faceJson.contains("tintindex")) {
                        if (!faceJson["tintindex"].is_number_integer()) return jsonutil::Fail<BlockModelDefinition>(debugName + ": face '" + dir + "' tintindex must be integer");
