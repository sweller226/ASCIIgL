...
float4 main(float4 svPos : SV_Position, float2 uv : TEXCOORD0) : SV_Target
{
    ...
    hr = impl_->_device->CreatePixelShader(psBlob->GetBufferPointer(), psBlob->GetBufferSize(), nullptr, &impl_->_asciiWindowPS);
    if (FAILED(hr)) {
    ...
    hr = impl_->_device->CreateBuffer(&cbd, nullptr, &impl_->_asciiWindowCB);
    if (FAILED(hr)) {
    ...
    hr = impl_->_device->CreateBuffer(&bufDesc, &initData, &impl_->_paletteBufferWindow);
    if (FAILED(hr)) {
    ...
    hr = impl_->_device->CreateShaderResourceView(impl_->_paletteBufferWindow.Get(), &paletteSrvDesc, &impl_->_paletteSRVWindow);
    if (FAILED(hr)) {
    ...
    // Must use the same impl_->_charRamp order as the font atlas (built in InitializeFontAtlas).
    if (impl_->_charRamp.size() != static_cast<size_t>(impl_->_fontAtlasGlyphCount)) {
        Logger::Error("[Renderer] ASCII window: char ramp size (" + std::to_string(impl_->_charRamp.size()) +
            ") != font atlas glyph count (" + std::to_string(impl_->_fontAtlasGlyphCount) + "); glyphs will be wrong.");
    }
    constexpr UINT rampLookupSize = 256;
    std::vector<uint32_t> rampLookupData(rampLookupSize, 0u);
    for (UINT cp = 0; cp < rampLookupSize; ++cp) {
        for (size_t j = 0; j < impl_->_charRamp.size(); ++j) {
            if (static_cast<unsigned>(impl_->_charRamp[j]) == cp) {
                rampLookupData[cp] = static_cast<uint32_t>(j);
                break;
            }
        }
    }
    Logger::Debug("[Renderer] Ramp lookup built: " + std::to_string(impl_->_charRamp.size()) + " glyphs, atlas slices " + std::to_string(impl_->_fontAtlasGlyphCount));
    ...
    hr = impl_->_device->CreateBuffer(&rampLookupBufDesc, &rampLookupInit, &impl_->_rampLookupBuffer);
    if (FAILED(hr)) {
    ...
    hr = impl_->_device->CreateShaderResourceView(impl_->_rampLookupBuffer.Get(), &rampLookupSrvDesc, &impl_->_rampLookupSRV);
    if (FAILED(hr)) {
...
void Renderer::RunAsciiWindowPass() {
    ...
    if (FAILED(impl_->_windowSwapChain->GetDesc(&scDesc))) {
    ...
    cbData.cellPixelsX      = static_cast<UINT>(impl_->_fontAtlasCellPixelsX);
    cbData.cellPixelsY      = static_cast<UINT>(impl_->_fontAtlasCellPixelsY);
    cbData.outWidth         = outWidth;
    cbData.outHeight        = outHeight;
    cbData.atlasSliceCount  = static_cast<UINT>(impl_->_fontAtlasGlyphCount);

    D3D11_MAPPED_SUBRESOURCE mapped;
    if (SUCCEEDED(impl_->_context->Map(impl_->_asciiWindowCB.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped))) {
        std::memcpy(mapped.pData, &cbData, sizeof(cbData));
        impl_->_context->Unmap(impl_->_asciiWindowCB.Get(), 0);
    }

    // Bind RTV for window back buffer
    ID3D11RenderTargetView* rtv = impl_->_windowRTV.Get();
    impl_->_context->OMSetRenderTargets(1, &rtv, nullptr);
    ...
    ID3D11ShaderResourceView* srvs[4] = { impl_->_charInfoSRV.Get(), impl_->_fontAtlasSRV.Get(), impl_->_paletteSRVWindow.Get(), impl_->_rampLookupSRV.Get() };
    impl_->_context->PSSetShaderResources(0, 4, srvs);

    ID3D11SamplerState* samplers[2] = { impl_->_fontAtlasSamplerPoint.Get(), impl_->_samplerLinear.Get() };
    impl_->_context->PSSetSamplers(0, 2, samplers);

    ID3D11Buffer* cb = impl_->_asciiWindowCB.Get();
    impl_->_context->VSSetConstantBuffers(0, 1, &cb);
    impl_->_context->PSSetConstantBuffers(0, 1, &cb);
    ...
    impl_->_context->PSSetShader(impl_->_asciiWindowPS.Get(), nullptr, 0);
