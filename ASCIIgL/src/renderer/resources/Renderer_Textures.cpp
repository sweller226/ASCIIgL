...
Renderer::GPUMeshCache* Renderer::GetOrCreateMeshCache(const Mesh* mesh) {
    ...
    if (mesh->GetVertexDataSize() > 0) {
        ...
        HRESULT hr = impl_->_device->CreateBuffer(&vbDesc, &vbData, &cache->vertexBuffer);
        if (FAILED(hr)) {
    ...
    if (!mesh->GetIndices().empty()) {
        ...
        HRESULT hr = impl_->_device->CreateBuffer(&ibDesc, &ibData, &cache->indexBuffer);
        if (FAILED(hr)) {
            std::ostringstream ss;
            ss << std::hex << hr;
...
void Renderer::InvalidateCachedTexture(const Texture* tex) {
    ...
    auto it = impl_->_textureCache.find(tex);
    if (it != impl_->_textureCache.end()) {
        impl_->_textureCache.erase(it);
...
bool Renderer::CreateTextureFromASCIIgLTexture(const Texture* tex, ID3D11ShaderResourceView** srv)
{
    ...
    HRESULT hr = impl_->_device->CreateTexture2D(&desc, nullptr, &texture);
    if (FAILED(hr)) {
    ...
    // -----------------------------
    {
        ...
        impl_->_context->UpdateSubresource(texture.Get(), 0, nullptr, gpuData.data(), baseW * 4, 0);
    }
    ...
    if (hasCustom) {
        for (int level = 1; level < mipCount; ++level) {
            ...
            impl_->_context->UpdateSubresource(
                texture.Get(),
                level,
                nullptr,
                gpuData.data(),
    ...
    // Create SRV
    // -----------------------------
    hr = impl_->_device->CreateShaderResourceView(texture.Get(), nullptr, srv);
    if (FAILED(hr)) {
    ...
    // -----------------------------
    if (!hasCustom) {
        impl_->_context->GenerateMips(*srv);
    }
...
void Renderer::BindTexture(const Texture* tex, int slot, SamplerType type) {
    ...
    if (it != impl_->_textureCache.end()) {
        ...
        srv = newSRV.Get();
        impl_->_textureCache[tex] = newSRV;
    }

    impl_->_context->PSSetShaderResources(slot, 1, &srv);
    impl_->_currentTextureSRV = srv;

    // Resolve Default: single texture → Point
    SamplerType resolved = (type == SamplerType::Default) ? SamplerType::Point : type;
    ID3D11SamplerState* sampler = (resolved == SamplerType::Anisotropic) ? impl_->_samplerAnisotropic.Get() : impl_->_samplerLinear.Get();
    impl_->_context->PSSetSamplers(slot, 1, &sampler);
}

void Renderer::UnbindTexture(int slot) {
    if (!impl_->_initialized) return;
    ID3D11ShaderResourceView* nullSRVs[] = { nullptr };
    impl_->_context->PSSetShaderResources(slot, 1, nullSRVs);
    if (slot == 0) impl_->_currentTextureSRV.Reset();
}
...
bool Renderer::CreateTextureArraySRV(const TextureArray* texArray, ID3D11ShaderResourceView** srv) {
    ...
    for (int layer = 0; layer < layerCount; ++layer) {
        if (hasCustom) {
            for (int mip = 0; mip < mipCount; ++mip) {
                ...
                impl_->_context->UpdateSubresource(texture.Get(), subresource, nullptr, data, mipW * 4, 0);
            }
        } else {
            ...
            impl_->_context->UpdateSubresource(texture.Get(), subresource, nullptr, data, tileSize * 4, 0);
        }
    }
    ...
    hr = impl_->_device->CreateShaderResourceView(texture.Get(), &srvDesc, srv);
    if (FAILED(hr)) {
...
void Renderer::BindTextureArray(const TextureArray* texArray, int slot, SamplerType type) {
    if (!impl_->_initialized || !texArray || !texArray->IsValid()) {
    ...
    auto it = impl_->_textureArrayCache.find(texArray);
    if (it != impl_->_textureArrayCache.end()) {
        srv = it->second;
    } else {
        ID3D11ShaderResourceView* rawSRV = nullptr;
        if (CreateTextureArraySRV(texArray, &rawSRV)) {
            srv.Attach(rawSRV);
            impl_->_textureArrayCache[texArray] = srv;
        }
    }
    ...
    if (srv) {
        ...
        impl_->_context->PSSetShaderResources(slot, 1, srvs);
