...
bool Shader::CompileFromSource(const std::string& source, const std::string& entryPoint,
    ...
    _impl->bytecode.Attach(shaderBlob);
    ...
    if (_type == ShaderType::Vertex) {
        hr = device->CreateVertexShader(
            _impl->bytecode->GetBufferPointer(),
            _impl->bytecode->GetBufferSize(),
            nullptr,
            &_impl->vertexShader
        );
    } else {
...
bool Shader::LoadFromBytecode(const std::vector<uint8_t>& bytecode) {
    ...
    HRESULT hr = D3DCreateBlob(bytecode.size(), &_impl->bytecode);
    if (FAILED(hr)) {
        _compileError = "Failed to create blob for bytecode";
        return false;
    }
    
    memcpy(_impl->bytecode->GetBufferPointer(), bytecode.data(), bytecode.size());
...
bool ShaderProgram::CreateInputLayout(const VertFormat& format) {
    ...
    HRESULT hr = device->CreateInputLayout(
        ...
        _vertexShader->_impl->bytecode->GetBufferPointer(),
        _vertexShader->_impl->bytecode->GetBufferSize(),
        &_impl->inputLayout
    );
